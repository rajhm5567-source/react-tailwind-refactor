...
export default function StudentManagement({ onLogOut, onViewChange }) {
          ...
          <main className="flex-1 p-[32px] overflow-y-auto bg-[#F8FAFC] flex flex-col gap-[24px]">
                ...
                <tbody className="divide-y divide-slate-100">
                  {students.map((student) => (
                    <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-[24px]">
                        <span className={`inline-flex items-center px-[12px] py-[4px] rounded-full text-[11px] font-medium ${student.status === 'Active' ? 'bg-[#F0FDF4] text-[#008236]' : 'bg-gray-100 text-gray-600'}`}>{student.status}</span>
                      </td>
                      <td className="p-[24px]">
                        <button onClick={() => handleGenerateCard(student.id)} className="px-[16px] py-[8px] bg-[#F3F4F6] text-[#364153] text-[13px] font-medium rounded-[10px] hover:bg-[#e4e6eb]">👁️ View Profile</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="bg-white rounded-[14px] p-[24px] shadow-sm border border-slate-100 flex flex-col gap-[16px]">
              ...
              <div className="flex gap-[16px]">
                ...
                />
                <button onClick={() => handleGenerateCard(searchId)} className="h-[48px] px-[32px] bg-[#8B0000] text-white font-bold text-[14px] rounded-lg hover:bg-red-950 transition-all">Generate Card</button>
              </div>
              ...
              {generatedCard && (
                <div className="mt-4 bg-slate-900 text-white p-[24px] rounded-[15px] max-w-[400px] border-l-8 border-[#8B0000] shadow-xl self-center w-full relative">
                  <button onClick={() => setGeneratedCard(null)} className="absolute top-3 right-3 text-gray-400 hover:text-white">✕</button>
